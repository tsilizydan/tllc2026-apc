    ...
    <nav className={`navbar${scrolled ? ' navbar--scrolled' : ''}`}>
      <div className="navbar__inner">
        ...
        <Link to="/" className="navbar__logo">
          <span className="logo__apos">APOS</span><span className="logo__apostrophe">'</span><span className="logo__creed">CREED</span>
        </Link>
        ...
        <div className="navbar__search">
          <div className="search-box">
            ...
            <input
              type="text" placeholder="Search games, accessories..."
              value={searchVal} onChange={e => setSearchVal(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && searchVal.trim()) { navigate(`/shop?search=${encodeURIComponent(searchVal.trim())}`); setSearchVal(''); setSearchRes([]) } }}
            />
            {searchVal && (
              <button className="search-clear" onClick={() => { setSearchVal(''); setSearchRes([]) }}>✕</button>
            )}
          </div>
          {searchRes.length > 0 && (
        ...
        <div className={`navbar__links${menuOpen ? ' open' : ''}`}>
          ...
          <Link to="/shop?category=pc-games" className="nav-link" onClick={() => setMenuOpen(false)}>PC</Link>
          <Link to="/shop?category=playstation" className="nav-link" onClick={() => setMenuOpen(false)}>PlayStation</Link>
          <Link to="/shop?category=accessories" className="nav-link" onClick={() => setMenuOpen(false)}>Accessories</Link>
        </div>
            ...
            <div className="user-menu" ref={userRef}>
              ...
              {userOpen && (
                <div className="user-dropdown">
                  ...
                  <Link to="/dashboard/profile" className="dropdown-item" onClick={() => setUserOpen(false)}>Profile</Link>
                  {user.role === 'admin' && (
